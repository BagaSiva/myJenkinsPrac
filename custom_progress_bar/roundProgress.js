...
angular.module('angular-svg-round-progress').service('roundProgressService', [function(){
    ...
    service.updateState = function(val, total, R, ring, size, isSemicircle, extraSize) {
    ...
    .directive('roundProgress', ['$window', 'roundProgressService', 'roundProgressConfig', function($window, service, roundProgressConfig){
            ...
            return angular.extend(base, {
                link: function(scope, element){
                    ...
                    var background  = svg.find('circle').eq(1);
                    var superBackground = svg.find('circle').eq(0);
                    ...
                    var renderCircle = function(){
                        ...
                        if(!isNested){
                            ...
                            svg[0].setAttribute('viewBox', '0 0 ' + (diameter * options.bgExtraSize) + ' ' + ((isSemicircle ? radius : diameter) * options.bgExtraSize));
                        ...
                        background.attr({
                            "cx":           radius * options.bgExtraSize,
                            "cy":           radius * options.bgExtraSize,
                            "r":            backgroundSize >= 0 ? backgroundSize : 0
                        }).css({
                            "stroke":       service.resolveColor(options.remainingColor),
                            "stroke-width": stroke
                        });
                        
                        superBackground.attr({
                             "cx":           radius * options.bgExtraSize,
                             "cy":           radius * options.bgExtraSize,
                             "r":            (backgroundSize >= 0 ? backgroundSize : 0) * options.bgExtraSize
                         }).css({
							 "fill" : service.resolveColor(options.bgcolor)
                         });
                    };
                    ...
                    var renderState = function(newValue, oldValue, preventAnimationOverride){
                        ...
                        var doAnimation = function(){
                            ...
                            if(preventAnimation){
                                service.updateState(end, max, circleSize, ring, elementSize, isSemicircle, options.bgExtraSize);
                                ...
                                (function animation(){
                                    ...
                                    service.updateState(animateTo, max, circleSize, ring, elementSize, isSemicircle, options.bgExtraSize);
